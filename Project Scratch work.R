...
#now beer2 is clean raw data if we need to use it later
...
#finalizing our clean, useable data
...
#need this for PCA
beer.clean.df<-column_to_rownames(beer.clean,var="beer_name")
head(beer.clean.df)

#need this for something else I think. If we want to use some form of logistic regression
beer.clean.fac_style<-beer.clean.df%>%mutate(beer_style=as.factor(beer_style))
head(beer.clean.fac_style)
...
styles<-distinct(beer.clean, beer_style)

#pairs plot, kinda cool looking, WARNING: will turn your macbook into a fighter-jet
ggpairs(beer.clean, columns=c(3,4,5,7,9,11,13),aes(color = beer_style, alpha = 0.5))
...
abv.plot<-ggplot(beer.clean, aes(beer_abv,review_overall, color=beer_style))
aroma.plot<-ggplot(beer.clean, aes(review_aroma,review_overall, color=beer_style))
appearance.plot<-ggplot(beer.clean, aes(review_appearance,review_overall, color=beer_style))
palate.plot<-ggplot(beer.clean, aes(review_palate,review_overall, color=beer_style))
taste.plot<-ggplot(beer.clean, aes(review_taste,review_overall, color=beer_style))

#abv vs overall review

abv.plot+geom_point()+geom_smooth(method = lm, se = FALSE)+
  theme(legend.position = "none")
abv.plot.ungrouped<-ggplot(beer.clean, aes(beer_abv,review_overall))
abv.plot.ungrouped+geom_point()+geom_smooth()+
  theme(legend.position = "none")

#R really doesn't want to make a heatmap, this will be figured out later
heatmap_data <- beer.clean[c('review_overall', 'review_aroma', 'review_appearance', 'review_palate', 'review_taste')]
...
set.seed(42069)
train<-sample(c(1:nrow(beer.clean)),floor(0.80 * nrow(beer.clean)))
test<- -train
beer.train<-beer.clean[train,]
beer.test<-beer.clean[test,]

#50/50 splot for training and testing data
set.seed(42069)
train<-sample(c(1:nrow(beer.clean)),floor(nrow(beer.clean)/2))
test<- -train
beer.train<-beer.clean[train,]
beer.test<-beer.clean[test,]